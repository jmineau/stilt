...
| `reset_output_wd` | Logical indicating whether to reset the output directory. If TRUE, the output directory contents are deleted. Setting to FALSE allows for multiple simulations to be run with the same output directory. Defaults to TRUE.                                                    |
...
| `write_trajec`  | Logical indicating whether to write the trajectory output to disk. Defaults to TRUE. If FALSE, the trajectory output is not saved to disk and is only available in the `output` object. This can be useful for reducing disk space when only the footprint is needed.           |
